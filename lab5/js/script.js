  ...
  var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
      if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        ...
        if(data["new_best"]== 1){
          $('#msg').html("Your solution is the highest record.<br><br>Best recorded from our database:<br> Number of matches: " + data["num_match"] + "<br>Score: " + data["match_score"]+ "<br>Best Time: " + data["best_time"]);
        }  
        else{
          $('#msg').html("Your solution is NOT the highest recorded. <br><br>Best recorded from our database:<br> Number of matches: " + data["num_match"] + "<br>Score: " + data["match_score"] + "<br>Best Time: " + data["best_time"] + "<br>Reset and try again :(");
        }  
        getOptimal();
      }
    ...
    xmlhttp.onreadystatechange = function() {
      if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        ...
        if(data["match_score"] == appState.totalScore && data["num_match"] == appState.numPenguins){
          $('#msg2').html("<br>Congratulations. You found the optimal solution");
        }
        else{
          $("#msg2").html("<br>Sorry, you didn't find the optimal score.");
        }
      }
    };
