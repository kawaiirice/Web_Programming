...
//insertTable(); //fill in the USER table 
...
function insertTable(){
	//put in USER table entries
	global $db;
	$query = "SELECT * FROM USER";
	$res = $db->query($query);

	$query = "INSERT INTO USER VALUES('boss', '" . crypt("TAplstest") . "', 0)";
	$res = $db->query($query);

	$query = "INSERT INTO USER VALUES('student1', '" . crypt("iamstudent1") . "' , 1)";
	$res = $db->query($query);

	$query = "INSERT INTO USER VALUES('student2', '" . crypt("number2") . "', 1)";
	$res = $db->query($query);
}
...
function checkDatabase ($graph_id, $num_match, $match_score, $best_time) {
		...
		// Compare values with our scoreboard
		...
		// Check if the user and hashed password is in our database
		$updatable = false;
		if($user_id != null && $hashed_password != null) {
			$query = "SELECT * FROM USER WHERE user_id = '" . $user_id . "' AND hashed_password = '" . $hashed_password . "'";
			$res = $db->query($query);
		...
		if($highest_match == $num_match && $highest_score == $match_score && $fastest_time == $best_time){
			...
			if($updatable){
				updateDatabase($graph_id, $num_match, $match_score, $best_time, $user_id);
				$a = array("new_best"=>1, "num_match"=>$num_match, "match_score"=>$match_score, "best_time"=>$best_time);
			}	
			else{
				$a = array("new_best"=>1, "num_match"=>$highest_match, "match_score"=>$highest_score, "best_time"=>$fastest_time);
			} 
		}
		else {
...
function updateDatabase ($graph_id, $num_match, $match_score, $best_time, $uid) {
	global $db;
	...
	$query = "UPDATE SCOREBOARD SET user_id='" . $uid . "' WHERE graph_id = '" . $graph_id . "'";
	$res = $db->query($query);
}
...
/* -------------------------------Hard coded graphs and an Optimal Solution----------------------------------- */
